...
using namespace std;
    ...
    bool set_address(const char* hname, const char* sname, struct sockaddr_in* sap, const char* protocol)
    {
		...
		//struct hostent hostEnt;// *pHostEnt;
    		//std::vector<char> buf(2048)
		///char buf[2048];
    	//	int err;
		//gethostbyname(hname, &hostEnt, &buf[0], 2048, &hp, &err);
                hp=gethostbyname(hname);
                if( hp == NULL )
                {
                    return false;
...
TRYAGAIN:
        ...
        if(iret  && errno==EADDRINUSE && trynum <5)
        {
            trynum++;
            sleep(trynum);
            cout<<"tcp_server bind failed tryagain errno is "<<errno<<endl;
            goto TRYAGAIN;
        }
        if(iret )
        {
            cout<<"tcp_server bind failed errno is "<<errno<<endl;
            ::close(fd);
            assert(0);
            return -1;
        }
        ...
        if( listen(fd, 500))
        {
            cout<<"tcp_server listen failed! errno is "<<errno<<endl;
            ::close(fd);
            assert(0);
            return -1;
        }
    ...
    int tcp_client( const char* hostIp,  const char* hostPort)
    {
        ...
        {
            cout << "tcp_client set_address call failed!" << errno << endl;
            return -1;
        }
        ...
        //		TRACEX(LEVEL_CRITICAL)<<"tcp_server setsockopt failed! errno is "<<errno<<endl;
        ...
        {
            cout << "tcp_client connect failed! errno is "<<errno<<endl;
            ::close( fd );				//added by helq.
            return -1;
        }
    ...
    int tcp_client2( const char* hostIp,  const char* hostPort)
    {
        ...
        if (!set_address( hostIp, hostPort, &peer, "tcp" ))
        {
            cout << "tcp_client set_address call failed!" <<endl;
            return -1;
        }
        ...
        {
            cout<<"tcp_client setsockopt failed! errno is "<<errno<<endl;
            close(fd);
            return -1;
        }
...
TRYAGAIN:
        ...
        if(iret  && errno==EADDRINUSE && trynum <5)
        {
            trynum++;
            sleep(trynum);
            cout<<"udp_server bind failed tryagain errno is "<<errno<<endl;
            goto TRYAGAIN;
        }
        if(iret )
        {
            cout<<"udp_server bind failed errno is "<<errno<<endl;
            ::close(fd);
            assert(0);
            return -1;
        }
    ...
    int udp_client( const char* hostIp,  const char* hostPort, struct sockaddr_in* local)
    {
        ...
        if ( fd<0 )
        {
            cout << "udp_client socket call failed! errno is "<<errno<<endl;
            return -1;
        }
